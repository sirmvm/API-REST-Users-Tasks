...
export default class UserRepository{
  public readonly findAll = async (): Promise<UserDTO[]> => {
    ...
    const usersWithoutPassword = users.map(user => {
      const {password, ...usersWithoutPassword} = user  //destructuring, trae todos los datos menos la password
  ...
  public readonly create = async (user:CreateUserDTO):Promise<UserDTO> => {
    ...
    const {password, ...usersWithoutPassword} = newUser  //destructuring, trae todos los datos menos la password
    return usersWithoutPassword
